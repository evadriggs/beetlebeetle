...
for (file in filenames) {
  ...
  Species = c(Species, rep(species, 5))
  ...
  # Calculate the remaining percentage for Non-repeat DNA
  first_index = counter
  last_index = counter + 3
  nonrepeat_percent = 100 - sum(Percentage[first_index:last_index])

  Percentage = c(Percentage, nonrepeat_percent)
...
custom_order = c("Non-repeat DNA", "Other repeats", "DNA transposons", 
                 "Retroelements", "Unclassified repeats")
custom_colors = rep(c("#Aeaaaf", "#1f3a4c", "#186372", "#17aeac", "#bb4135"), 
                    length(filenames))
...
p = ggplot(data, aes(fill = Category, y = Percentage, x = Species)) +
  ...
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 20)) +   # Scale y axis to 100
  labs(
...
p + geom_text(aes(label = Percentage_label),
